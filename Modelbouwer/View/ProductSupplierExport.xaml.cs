...
/// <summary>
/// Interaction logic for ProductSupplierExport.xaml
/// </summary>
public partial class ProductSupplierExport : Page
{
	private DataTable? _dt;
	public ProductSupplierExport()
	{
		InitializeComponent();
		GetData();
	...
	{
		_dt = DBCommands.GetData( DBNames.ProductSupplierTable, "nosort" );

		if ( _dt == null )
		{
			dispFolderName.Text = TryFindResource( "Export.ProductSupplier.Empty" ) as string;
			dispStatusLine.Text = TryFindResource( "Export.ProductSupplier.Empty" ) as string;
	...
	{
		string _filename = $"{GeneralHelper.GetFilePrefix()}{(string)FindResource("Export.ProductSupplier.Filename")}.csv";
