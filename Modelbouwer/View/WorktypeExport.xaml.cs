...
/// <summary>
/// Interaction logic for WorktypeExport.xaml
/// </summary>
public partial class WorktypeExport : Page
{
	private DataTable? _dt;

	public WorktypeExport()
	{
		InitializeComponent();
		GetData();
	...
	{
		_dt = DBCommands.GetData( DBNames.WorktypeTable, "nosort" );

		if ( _dt == null )
		{
			dispFolderName.Text = TryFindResource( "Export.WorkType.Empty" ) as string;
			dispStatusLine.Text = TryFindResource( "Export.WorkType.Empty" ) as string;
	...
	{
		string _filename = $"{GeneralHelper.GetFilePrefix()}{(string)FindResource("Export.WorkType.Filename")}.csv";
