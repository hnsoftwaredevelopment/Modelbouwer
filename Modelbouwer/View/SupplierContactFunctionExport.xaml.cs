...
/// <summary>
/// Interaction logic for SupplierContactFunctionExport.xaml
/// </summary>
public partial class SupplierContactFunctionExport : Page
{
	private DataTable? _dt;

	public SupplierContactFunctionExport()
	{
		InitializeComponent();
		GetData();
	...
	{
		_dt = DBCommands.GetData( DBNames.ContactTypeTable, "nosort" );

		if ( _dt == null )
		{
			dispFolderName.Text = TryFindResource( "Export.SupplierContactFunction.Empty" ) as string;
			dispStatusLine.Text = TryFindResource( "Export.SupplierContactFunction.Empty" ) as string;
	...
	{
		string _filename = $"{GeneralHelper.GetFilePrefix()}{(string)FindResource("Export.SupplierContactFunction.Filename")}.csv";
