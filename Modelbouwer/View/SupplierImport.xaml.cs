...
public partial class SupplierImport : Page
{
	...
	private void Prepare( object sender, RoutedEventArgs e )
	{
		...
		var _filename = $"{GeneralHelper.GetFilePrefix()}{(string)FindResource("Import.Supplier.Filename")}.csv";
	...
	private void SelectFile( object sender, RoutedEventArgs e )
	{
		OpenFileDialog fileDialog = new ()
		{
			...
			DefaultExt = ".csv",
			Filter = $"{(string)FindResource("Import.Supplier.FileDialog.FilterTest")}  (*.csv)|*.csv",
			FilterIndex = 1
		};
	...
	{
		var errorIdentifier = 9; //See Error Class for meaning of this number
