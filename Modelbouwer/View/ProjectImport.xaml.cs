...
/// <summary>
/// Interaction logic for ProjectImport.xaml
/// </summary>
public partial class ProjectImport : Page
{
	...
	private void Prepare( object sender, RoutedEventArgs e )
	{
		...
		string _filename = $"{GeneralHelper.GetFilePrefix()}{(string)FindResource("Import.Project.Filename")}.csv";
	...
	{
		int errorIdentifier = 7; //See Error Class for meaning of this number
		string[] checkField = [DBNames.ProjectFieldNameCode, DBNames.ProjectFieldTypeCode, "0"]; // The field name to check if record is unique, number is the header item that contains thye data for the check field
		string metadataType = "Project"; // Used for getting the headers for import

		dispStatusLine.Text = GeneralHelper.ProcessCsvFile( DBNames.ProjectTable, errorIdentifier, dispFileName.Text, checkField, metadataType );
	}
	#endregion
